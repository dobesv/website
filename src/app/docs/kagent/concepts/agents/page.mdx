...
When a container-based skill is loaded, the agent is provided with a tool matching the skill's name with outputs the SKILL.md with instructions.  It will also provide tools read_file, write_file, edit_file, bash.  The agent can use `bash` to run scripts provided by the agent in the container.  The agent can use the file related tools to work with files included in the skill container, or files generated by a script that it ran or created in the skill container.  Note: be careful not to have any other tools with the same name as one of these if you are using container skills as the LLM will like fail with a duplicate tool name error.
